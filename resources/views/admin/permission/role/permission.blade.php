                                                ...
                                                @if(hasPermission($value['group_code'],VIEW) && hasActive($value['group_code']))
                                                    <tr>
                                                        ...
                                                        @if (!empty($value['permission']))
                                                                ...
                                                                <td>
                                                                    @if(($value['permission'][0]->enable_publish == 1) && hasPermission($value['permission'][0]->short_code,PUBLISH))
                                                                    <label class="">
                                                                        <input type="checkbox" class="group_{{$value['group_code']}}" name="{{"can_publish-perm_" . $value['permission'][0]->pc_id}}" value="{{$value['permission'][0]->pc_id}}" {{$value['permission'][0]->can_publish == 1?"checked":""}}>
                                                                    </label>
                                                                    @endif
                                                                </td>
                                                        @else
                                                        ...
                                                        @foreach($value["permission"] as $new_feature_key => $new_feature_value)
                                                            @if(hasPermission($new_feature_value->short_code,VIEW))
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        @if($new_feature_value->enable_publish == 1  && hasPermission($new_feature_value->short_code,PUBLISH))
                                                                        <label class="">
                                                                            <input type="checkbox" class="group_{{$value['group_code']}}" name="{{"can_publish-perm_" . $new_feature_value->pc_id}}" value="{{$new_feature_value->pc_id}}" {{$new_feature_value->can_publish == 1?"checked":""}}>
                                                                        </label>
                                                                        @endif
                                                                    </td>
                                                                </tr>
